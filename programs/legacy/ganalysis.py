...
class Grams(Corpus):
    ...
    def getWords(self):
        ...
        for (w, word) in enumerate(words):
            WORD_OCCS[word].append(w + 1)
        print(f"{len(WORD_OCCS)} words in {len(words)} occurrences")
    ...
    def getGramContexts(self):
        ...
        for n in NS:
            ...
            flex = k + n
            for (w, word) in enumerate(words):
                prevWord = "" if w == 0 else words[w - 1]
                postWord = "" if w == len(words) - 1 else words[w + 1]
                before = (" " * flex) + prevWord
                after = postWord + (" " * flex)
    ...
    def getGrams(self):
        ...
        *   `GRAM_INDEX["word"][n]` gives per word a dict
        ...
        print(f"Getting 2,3-grams for {len(words)} words in volume {volume}")
